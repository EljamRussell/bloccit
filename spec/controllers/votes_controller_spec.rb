...
RSpec.describe VotesController, type: :controller do
  ...
  # test that unsigned in users are redirected to the sign-in page
  # they are not allowed to vote on posts
  context "guest" do
  ...
  context "signed in user" do
    ...
    describe "POST up_vote" do
      # expect first time user up botes a post, a new vote is created for that post
      ...
      # test that new vote is not created if user repeatedly up votes a post
      ...
      # expect that up voting will increase the number of points on that post by one
      ...
      # test that users are redirected back to correct view
      # post show or topics show, depending on which view they voted from
      # by setting request.env["HTTP_REFERER"] to th requesting URL
